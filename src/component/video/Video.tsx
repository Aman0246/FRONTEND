...
const Video: React.FC<VideoProps> = (props) => {
  ...
  const { realVideo, totalRealVideo } = useSelector((state: RootStore) => state.video);
  const { dialogueType, dialogueData } = useSelector((state: RootStore) => state.dialogue);
  ...
  const videoTable = [
    ...
    },
    {
      ...
      Cell: ({ row }: { row: any }) => (
        <div
          className="text-capitalize userText d-flex align-items-center"
          style={{
            cursor: "pointer",
          }}
    ...
    },
    {
      Header: "Unique ID",
      body: "uniqueId",
      Cell: ({ row }: { row: any }) => <span className="text-capitalize">{row?.uniqueVideoId}</span>,
    },
    ...
    },
    {
      ...
      Cell: ({ row }: { row: any }) => (
        <>
          <button className="viewbutton mx-auto" onClick={() => dispatch(openDialog({ type: "viewVideo", data: row }))}>
            <IconVideo />
          </button>
        </>
      ),
  ...
  return (
    <div>
      <div className="user-table mb-3">
        ...
        <Table data={data} mapData={videoTable} serverPerPage={size} serverPage={page} handleSelectAll={handleSelectAll} selectAllChecked={selectAllChecked} type={"server"} />
        <div className="mt-3">
          <Pagination type={"server"} activePage={page} rowsPerPage={size} userTotal={totalRealVideo} setPage={setPage} handleRowsPerPage={handleRowsPerPage} handlePageChange={handlePageChange} />
        </div>
      </div>
    </div>
  );
