...
const FakeVideo: React.FC<FakeVideoProps> = (props) => {
  ...
  const { fakeVideoData, totalFakeVideo } = useSelector((state: RootStore) => state.video);
  ...
  return (
    <div>
      <div className="user-table mb-3">
        ...
        <div className="mt-3">
          <Pagination type={"server"} activePage={page} rowsPerPage={size} userTotal={totalFakeVideo} setPage={setPage} handleRowsPerPage={handleRowsPerPage} handlePageChange={handlePageChange} />
        </div>
      </div>
    </div>
  );
