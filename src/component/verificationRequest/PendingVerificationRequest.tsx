...
import { getVerificationRequest, verificationRequestAccept, verificationRequestDecline } from "../../store/verificationRequestSlice";
    ...
    <>
      ...
      <Dialog open={open} onClose={handleClose} maxWidth="md">
        ...
        <DialogContent style={{ padding: 0 }}>
          <img src={src} style={{ width: "100%", height: "100%" }} alt="full" />
        </DialogContent>
      </Dialog>
    </>
  );
...
const PendingVerificationRequest = () => {
  ...
  const { pendingData, totalPendingData } = useSelector((state: RootStore) => state.verificationRequest);
  ...
  const postReportTable = [
...

    {
      Header: "Date",
      body: "date",
      Cell: ({ row }: { row: any }) => <span className="text-capitalize">{row?.date ? dayjs(row?.date).format("DD MMMM YYYY") : ""}</span>,
    },
    {
      Header: "Action",
      body: "action",
      Cell: ({ row }: { row: any }) => (
        <div className="action-button">
          {row.status === 2 ? "" : <Button btnIcon={<IconCheck className="text-secondary" />} onClick={() => handleEdit(row?._id, "pay")} />}
          <Button btnIcon={<IconX className="text-secondary" />} onClick={() => handleDecline(row?._id)} />
        </div>
      ),
    },
  ];
