...
const Story: React.FC<VideoProps> = (props) => {
  ...
  const { realStory, totalRealStory } = useSelector((state: RootStore) => state.story);
    ...
    },
    {
      ...
      Cell: ({ row }: { row: any }) => (
        <div className="action-button">
          ...
          />
          <div className="action-button">
            <Button btnIcon={<IconTrash className="text-secondary" />} onClick={() => handleDeleteStory(row)} />
          </div>
        </div>
      ),
    },
  ...
  return (
    <div>
      <div className="user-table mb-3">
        ...
        <div className="mt-3">
          <Pagination type={"server"} activePage={page} rowsPerPage={size} userTotal={totalRealStory} setPage={setPage} handleRowsPerPage={handleRowsPerPage} handlePageChange={handlePageChange} />
        </div>
      </div>
    </div>
  );
