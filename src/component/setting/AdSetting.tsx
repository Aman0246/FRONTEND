...
import { androidAdsContent, iosAdsContent } from "@/extra/infoContent";
...
interface AdSettingValues {
  androidGoogleInterstitial: string;
  googleNative: string;
  googleNativeAdVideo: string;
  iosInterstital: string;
  iosNative: string;
  iosNativeAdVideo: string;
  adDisplayIndex: string | number;
}

const AdSetting: React.FC = () => {
  ...
  const [androidGoogleInterstitial, setAndroidGoogleInterstitial] =
    useState("");
  const [googleNative, setGoogleNative] = useState<string>("");
  const [googleNativeAdVideo, setGoogleNativeAdVideo] = useState<string>("");
  const [iosInterstital, setIosInterstial] = useState<string>("");
  const [iosNative, setIosNative] = useState<string>("");
  const [iosNativeAdVideo, setIosNativeAdVideo] = useState<string>("");
  const [isGoogle, setIsGoogle] = useState();
  const [adDisplayIndex, setAdDisplayIndex] = useState<string | number>("");
  ...
  const [initialValues, setInitialValues] = useState<AdSettingValues>({
  ...
  useEffect(() => {
    ...
    setGoogleNativeAdVideo(data?.android?.google?.nativeAdVideo);
    setIosInterstial(data?.ios?.google?.interstitial);
    setIosNative(data?.ios?.google?.native);
    setIosNativeAdVideo(data?.ios?.google?.nativeAdVideo);
    ...
    setInitialValues({
      androidGoogleInterstitial: data?.android?.google?.interstitial || "",
      googleNative: data?.android?.google?.native || "",
      googleNativeAdVideo: data?.android?.google?.nativeAdVideo || "",
      iosInterstital: data?.ios?.google?.interstitial || "",
      iosNative: data?.ios?.google?.native || "",
      iosNativeAdVideo: data?.ios?.google?.nativeAdVideo || "",
      adDisplayIndex: data?.adDisplayIndex || "",
    });
  }, [data]);
  ...
  const handleSubmit = () => {
    ...
    const currentValues: AdSettingValues = {
      androidGoogleInterstitial,
      googleNative,
      googleNativeAdVideo,
      iosInterstital,
    ...
    const hasChanges = Object.keys(currentValues).some(
      (key) =>
        currentValues[key as keyof AdSettingValues] !==
        initialValues[key as keyof AdSettingValues]
    );
    ...
    if (
      currentValues.androidGoogleInterstitial !==
      initialValues.androidGoogleInterstitial
    ) {
      changedData.androidGoogleInterstitial = androidGoogleInterstitial;
    }
    if (currentValues.googleNative !== initialValues.googleNative) {
      changedData.androidGoogleNative = googleNative;
    }
    if (
      currentValues.googleNativeAdVideo !== initialValues.googleNativeAdVideo
    ) {
      changedData.androidGoogleNativeAdVideo = googleNativeAdVideo;
    }
    if (currentValues.iosInterstital !== initialValues.iosInterstital) {
      changedData.iosGoogleInterstitial = iosInterstital;
    }
    if (currentValues.iosNative !== initialValues.iosNative) {
      changedData.iosGoogleNative = iosNative;
    }
    if (currentValues.iosNativeAdVideo !== initialValues.iosNativeAdVideo) {
      changedData.iosGoogleNativeAdVideo = iosNativeAdVideo;
    }
    if (currentValues.adDisplayIndex !== initialValues.adDisplayIndex) {
      changedData.adDisplayIndex = adDisplayIndex;
    }
          ...
          <div className="col-6 col-lg-6">
            <div className="mb-4">
              <div className="withdrawal-box border payment-box">
                <h6 className="border-bottom pb-3 d-flex align-items-center justify-content-between">
                  Google Ads Setting
                  <InfoTooltip
                    title="Google Ads Setting"
                    content={androidAdsContent}
                  />
                </h6>
                <div className="row">
            ...
            <div className="mb-4">
              <div className="withdrawal-box payment-box">
                ...
                <div className="col-12 withdrawal-input border-setting">
                  <Input
                    label={"Android Google Native Ad Video"}
                    name={"androidGoogleNativeAdVideo"}
                    type={"text"}
                    value={googleNativeAdVideo}
                    placeholder={"Google Native Ad Video"
                    }
                    onChange={(e) => {
                      setGoogleNativeAdVideo(e.target.value);
                    }}
                  />
                </div>
              </div>
        ...
        <div className="payment-setting row p-3">
            ...
            <div className="mb-4">
              <div className="withdrawal-box payment-box">
                <h6 className="d-flex align-items-center justify-content-between">
                  IOS
                  <InfoTooltip
                    title="IOS Ads Setting"
                    content={iosAdsContent}
                  />
                </h6>
                <div className="col-12 withdrawal-input border-setting">
                  <Input
                ...
                <div className="col-12 withdrawal-input border-setting">
                  <Input
                    label={"IOS Google Native Ad Video"}
                    name={"iosGoogleNativeAdVideo"}
                    type={"text"}
                    value={iosNativeAdVideo}
                    placeholder={
                      "IOS Google Native Ad Video"
                    }
                    onChange={(e) => {
                      setIosNativeAdVideo(e.target.value);
                    }}
                  />
                </div>
              </div>
          ...
          <div className="col-md-6">
            <div className="mb-4">
              <div className="withdrawal-box payment-box">
                <h6 className="d-flex align-items-center justify-content-between">
                  Ads Displayed After a Variable Number of Videos
                  <InfoTooltip
                ...
                <div className="col-12 withdrawal-input border-setting">
                  <Input
                    ...
                    placeholder={
                      "Ad Display Frequency (Number of Videos)"
                    }
                    // errorMessage={
                    //   error.iosGoogleInterstitial && error.iosGoogleInterstitial
                    // }
