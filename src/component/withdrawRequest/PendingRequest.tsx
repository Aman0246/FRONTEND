...
import { getwithdrawRequest, withdrawRequestAccept, withdrawRequestDecline } from "@/store/withdrawRequestSlice";
...
const PendingRequest = (props: any) => {
  const { pendingData, totalPendingData } = useSelector((state: RootStore) => state.withdrawRequest);
  ...
  const ManageUserData = [
...

    {
      ...
      Cell: ({ row }) => (
        ...
        <div className="action-button">
          <CustomButton btnIcon={<IconInfoCircle className="text-secondary" />} onClick={() => handleOpenInfo(row)} />
        </div>
      ),
    },

    {
      ...
      Cell: ({ row }) => (
        <div className="action-button">
          <CustomButton btnIcon={<IconCircleCheck className="text-success" />} onClick={() => handleEdit(row, "pay")} />

          <CustomButton btnIcon={<IconCircleX className="text-danger" />} onClick={() => handleDecline(row?._id)} />
        </div>
      ),
    },
  ];
  ...
  return (
    <>
      ...
      <div className="user-table real-user mb-3">
        ...
        <Pagination type={"server"} activePage={page} rowsPerPage={size} userTotal={totalPendingData} setPage={setPage} handleRowsPerPage={handleRowsPerPage} handlePageChange={handlePageChange} />
      </div>
            ...
            <form>
              <div className="row sound-add-box" style={{ overflowX: "hidden" }}>
                <div className="col-12 text-about">
                  ...
                  ></textarea>
                  {error.reason && <p className="errorMessage">{error.reason && error.reason}</p>}
                </div>
              </div>
            </form>
          </div>
      ...
      <Modal open={isAccept} onClose={handleIsAccept} aria-labelledby="modal-modal-title" aria-describedby="modal-modal-description">
        <Box sx={style} className="">
          <div className="model-body">
            <p className="fs-4 fw-medium m-0 my-4 text-center ">Would you like to approve the withdraw request?</p>
          </div>

          <div className="model-footer">
            <div className="p-3 d-flex justify-content-end">
              <Button onClick={handleIsAccept} btnName={"Close"} newClass={"close-model-btn"} />
              <Button
            ...
            <form>
              <div className="row sound-add-box" style={{ overflowX: "hidden" }}>
                <div className="col-12 ">
                  <div className="col-12  text-about">
                    <Input type={"text"} label={"Payment Gateway"} name={"Payment Gateway"} value={infoData?.paymentGateway} newClass={``} readOnly />
                  </div>

                  {infoData?.paymentDetails?.map((item) => {
                    return (
                      <div className="col-12 mt-1 text-about">
                        <Input type={"text"} label={item?.split(":")[0]} name={"Payment Details"} value={item?.split(":")[1]} newClass={`mt-3`} readOnly />
                      </div>
                    );
                  })}
                </div>
