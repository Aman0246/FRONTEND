...
const VerifiedUser = (props) => {
  ...
  const { verifiedUserData, totalVerifiedUser } = useSelector((state: RootStore) => state.user);
  ...
  }, [dispatch, startDate, endDate, page, size, search, blockFilter]);
    ...
    },
    {
      ...
      Cell: ({ row }) => (
        <div className="d-flex align-items-center " style={{ cursor: "pointer" }} onClick={() => handleEdit(row, "manageUser")}>
          ...
          />
          <span className="text-capitalize fw-bold ms-3 cursorPointer text-nowrap">{row?.name}</span>
          <img src="/images/verified.png" alt="Edit Icon" className="ms-1" width={18} height={18} />
        </div>
      ),
    },
    {
  ...
  return (
    <div>
      <div className="user-table real-user mb-3">
        ...
        <Pagination type={"server"} activePage={page} rowsPerPage={size} userTotal={totalVerifiedUser} setPage={setPage} handleRowsPerPage={handleRowsPerPage} handlePageChange={handlePageChange} />
      </div>
    </div>
  );
};
