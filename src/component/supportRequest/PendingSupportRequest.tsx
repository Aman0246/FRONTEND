...
const PendingSupportRequest = (props: any) => {
  const { pendingData, totalPendingData } = useSelector((state: RootStore) => state.support);
  ...
  const ManageUserData = [
...

    {
      ...
      Cell: ({ row }) => (
        <div className="action-button">
          <Button btnIcon={<IconInfoCircle className="text-secondary" />} newClass={`fw-bolder text-white`} onClick={() => handleOpenInfo(row)} />
        </div>
        // <img
        //   src={infoImage.src}
        //   alt="Info"
...

    {
      ...
      Cell: ({ row }) => (
        <div className="action-button">
          <Button btnIcon={<IconCircleDashedCheck className="text-secondary" />} newClass={`fw-bolder text-white`} onClick={() => handleEdit(row, "pay")} />
          <CustomButton btnIcon={<IconTrash className="text-secondary" />} onClick={() => handleDeleteSupportRequest(row?._id)} />
        </div>
      ),
    },
  ];
                    ...
                    return (
                      <div className="col-12 mt-2">
                        <div className="col-12 mt-3 text-about">
                          <Textarea type={"text"} label={"Description"} name={"description"} value={info?.complaint} newClass={`mt-3`} row={4} readOnly />
                        </div>

                        <div className="col-12 mt-1 text-about">
                          <Input type={"text"} label={"Contact number"} name={"Contact number"} value={info?.contact} newClass={`mt-3`} readOnly />
                        </div>
