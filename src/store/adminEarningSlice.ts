...
export const getAdminEarning: any = createAsyncThunk("admin/coinPlan/fetchUserCoinplanTransactions", async (payload: AllUsersPayload | undefined) => {
  return apiInstanceFetch.get(
    `admin/coinPlan/fetchUserCoinplanTransactions?startDate=${payload?.startDate}&endDate=${payload?.endDate}&start=${payload?.page}&limit=${payload?.size}&search=${payload?.search}`
  );
});
