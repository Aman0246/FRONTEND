...
export const getReport = createAsyncThunk(
  ...
  async (payload: AllReportPayload | undefined) => {
    return apiInstanceFetch.get(
      `admin/report/getReports?start=${payload?.start}&limit=${payload?.limit}&type=${payload?.type}&status=${payload?.status}&startDate=${payload?.startDate}&endDate=${payload?.endDate}&search=${payload?.search}`
    );
  }
);
...
const reportReducer = createSlice({
  ...
  extraReducers: (builder) => {
    builder.addCase(getReport.pending, (state, action: PayloadAction<any>) => {
      ...
      state.videoReport = [];
      state.postReport = [];
      state.totalUserReport = 0;
      state.totalVideoReport = 0;
      state.totalPostReport = 0;
    });
