...
export const allVideo = createAsyncThunk(
  ...
  async (payload: AllVideoPayload | undefined) => {
    return apiInstanceFetch.get(
      `admin/video/getVideos?start=${payload?.start}&limit=${payload?.limit}&startDate=${payload?.startDate}&endDate=${payload?.endDate}&type=${payload?.type}&search=${payload?.search}`
    );
  }
);
