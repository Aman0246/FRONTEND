...
export const allUsers = createAsyncThunk(
  ...
  async (payload: AllUsersPayload | undefined) => {
    return apiInstanceFetch.get(
      `admin/user/getUsers?start=${payload?.start}&limit=${payload?.limit}&startDate=${payload?.startDate}&endDate=${payload?.endDate}&type=${payload?.type}&search=${payload?.search}&isBlock=${payload?.isBlock}`
    );
  }
);
...
export const getCountry = createAsyncThunk(
  "https://restcountries.com/v3.1/all?fields=name&fields=flags",
  async () => {
    const response = await fetch("https://restcountries.com/v3.1/all?fields=name&fields=flags");
