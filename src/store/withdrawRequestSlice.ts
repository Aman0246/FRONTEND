...
export const getwithdrawRequest = createAsyncThunk(
  ...
  async (payload: AllWithdrawPayload | undefined) => {
    return apiInstanceFetch.get(
      `admin/withdrawRequest/index?start=${payload?.start}&limit=${payload?.limit}&type=${payload?.type}&startDate=${payload?.startDate}&endDate=${payload?.endDate}&search=${payload?.search}`
    );
  }
);
